...
${TOP_DIR}/bin/operator-sdk build "$OSDK_INTEGRATION_IMAGE"
...
olm_latest_exists=0
if ! ${TOP_DIR}/bin/operator-sdk olm status >/dev/null 2>&1; then
	${TOP_DIR}/bin/operator-sdk olm install
	olm_latest_exists=1
fi
...
if eval "(( $olm_latest_exists ))"; then
	${TOP_DIR}/bin/operator-sdk olm uninstall
fi
